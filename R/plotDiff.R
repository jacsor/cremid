...
#'
#'
#' @param object Object of class inheriting from "cremid".
#' @return A plot. 
#' @export
#' @examples
#' 
...
{
  if(class(object)!="cremid")
  {
    print("ERROR: Input's class should be cremid")
    return(0)
  }
  ...
  if(type == "weight") {
    for( it in 1:nrow(state))
      state[it,] = ( object$chain$R[it, object$chain$Z[it,]] == 1)
  }  else if(type == "shift")
  {
    state = matrix(NA, nrow=nrow(object$chain$Z),  ncol=ncol(object$chain$Z))
    for( it in 1:nrow(state))
